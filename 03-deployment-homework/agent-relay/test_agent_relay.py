...
if not os.getenv("RELAY_DATABASE_URL") and not os.getenv("DATABASE_URL"):
    scratch_db = Path(tempfile.gettempdir()) / "agent-relay-test.db"
    os.environ["RELAY_DATABASE_URL"] = f"sqlite:///{scratch_db.as_posix()}"
...
def test_agent_exchange_round_trip_task_and_result():
    with TestClient(main.app) as client:
        ...
        created = client.post(
            ...
            json={"to": recipient["agent_id"], "input": "Uppercase this: hello"},
        )
        ...
        assert claim_data["input"] == "Uppercase this: hello"

        completed = client.post(
            f"/api/v1/tasks/{task_id}/complete",
            headers=recipient_headers,
            json={"claim_token": claim_data["claim_token"], "output": "HELLO"},
        )
